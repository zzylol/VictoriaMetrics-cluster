...
The following `tip` changes can be tested by building VictoriaLogs from the latest commit of [VictoriaMetrics](https://github.com/zzylol/VictoriaMetrics-cluster/) repository
...
* FEATURE: [HTTP querying APIs](https://docs.victoriametrics.com/victorialogs/querying/#http-api): allow passing arbitrary [LogsQL filters](https://docs.victoriametrics.com/victorialogs/logsql/#filters) to `extra_filters` and `extra_stream_filters` query args. See [these docs](https://docs.victoriametrics.com/victorialogs/querying/#extra-filters) and [this feature request](https://github.com/zzylol/VictoriaMetrics-cluster/issues/5542) for details.
* FEATURE: [Grafana Loki data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/promtail/): add support of Loki healthcheck `/insert/ready` endpoint. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7824).

* BUGFIX: [syslog data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/): correctly parse rows with multiple consecutive spaces between fields. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7776).
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix cursor reset in query input field. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7288).

## [v1.3.2](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.3.2-victorialogs)
...
* BUGFIX: [`stream_context` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#stream_context-pipe): fix `index out of range [0] with length 0` panic, which has been introduced in [v1.3.0-victorialogs](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.3.0-victorialogs). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7762).

## [v1.3.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.3.1-victorialogs)

Released at 2024-12-08

* BUGFIX: [`facets` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#facets-pipe): fix `assignment to entry in nil map` panic, which has been introduced in [v1.3.0-victorialogs](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.3.0-victorialogs).

## [v1.3.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.3.0-victorialogs)
...
* BUGFIX: [`/select/logsql/stats_query`](https://docs.victoriametrics.com/victorialogs/querying/#querying-log-stats): properly apply `limit` at [`stats` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#stats-pipe). The bug was introduced in the release [v1.1.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.1.0-victorialogs) when fixing [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7699).
...
## [v1.1.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.1.0-victorialogs)
...
* FEATURE: [format pipe](https://docs.victoriametrics.com/victorialogs/logsql/#format-pipe): allow formatting log fields in lowercase and uppercase via `<uc:field_name>` and `<lc:field_name>` syntax. This can be useful when some fields must be consistently transformed to the same case during querying. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7620#issuecomment-2502170924).
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): reduce memory usage across all tabs for improved performance and stability. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7185).
...
* BUGFIX: [`/select/logsql/stats_query_range` API](https://docs.victoriametrics.com/victorialogs/querying/#querying-log-range-stats): properly handle [`limit` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#limit-pipe) after [`sort` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#sort-pipe). Previously the `limit` was applied globally across all the calculated stats, while it must be applied individually per each `step` on the `start ... end` time range. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7699).
* BUGFIX: [vmui](https://docs.victoriametrics.com/#vmui): fix for `showLegend` and `alias` flags in predefined panels. [See this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7565)
* BUGFIX: fix `too big number of columns detected in the block` panic when the ingested logs contain more than 2000 fields with different names per every [log stream](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7568) for details.
...
## [v1.0.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v1.0.0-victorialogs)

Released at 2024-11-12

This release is identical to [v0.42.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.42.0-victorialogs).

VictoriaLogs gained all [the planned features](https://docs.victoriametrics.com/victorialogs/)
since the [initial v0.1.0 release](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.1.0-victorialogs) 1.5 years ago,
and is ready for production!

## [v0.42.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.42.0-victorialogs)
...
* FEATURE: [`/select/logsql/tail` HTTP endpoint](): support for `offset` query arg, which can be used for delayed emission of matching logs during live tailing. Thanks to @Fusl for the initial idea and implementation in [this pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/7428).
...
* FEATURE: [syslog data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/): allow adding the specified [log fields](https://docs.victoriametrics.com/victorialogs/keyconcepts/#data-model) during data ingestion. See [these docs](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/#adding-extra-fields). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7354).
* FEATURE: [Loki data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/#loki-json-api): show the original request body on parse errors. This should simplify debugging. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7490).

* BUGFIX: [`values` stats function](https://docs.victoriametrics.com/victorialogs/logsql/#values-stats): fix a bug, which could lead to corrupted results. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7458).
* BUGFIX: [`uniq_values`](https://docs.victoriametrics.com/victorialogs/logsql/#uniq_values-stats), [`min`](https://docs.victoriametrics.com/victorialogs/logsql/#min-stats), [`max`](https://docs.victoriametrics.com/victorialogs/logsql/#max-stats), [`row_min`](https://docs.victoriametrics.com/victorialogs/logsql/#row_min-stats) and [`row_max`](https://docs.victoriametrics.com/victorialogs/logsql/#row_max-stats) stats functions: fix a bug, which could return non-matching field values for these functions. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7458).
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add sorting of logs by groups and within each group by time in desc order. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7184) and [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7045).
* FEATURE: add support for receiving DataDog logs over network. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6632).
...
## [v0.40.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.40.0-victorialogs)

Released at 2024-10-31

* FEATURE: add support for extra filters across all the [HTTP querying APIs](https://docs.victoriametrics.com/victorialogs/querying/#http-api). See [these docs](https://docs.victoriametrics.com/victorialogs/querying/#extra-filters) for details. This is needed for implementing quick filtering on field values at [this feature request](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7365).

* BUGFIX: properly apply [`replace`](https://docs.victoriametrics.com/victorialogs/logsql/#replace-pipe) and [`replace_regexp`](https://docs.victoriametrics.com/victorialogs/logsql/#replace_regexp-pipe) pipes to identical values in adjacent log entries. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7162).
...
## [v0.39.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.39.0-victorialogs)
...
* BUGFIX: fix `runtime error: index out of range [0] with length 0` panic during low-rate data ingestion. The panic has been introduced in [v0.38.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.38.0-victorialogs). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7391).

## [v0.38.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.38.0-victorialogs)

Released at 2024-10-29

* FEATURE: added the ability to receive systemd (journald) logs over network. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4618).
...
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix various glitches with updating query responses. The issue was introduced in [v0.36.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.36.0-victorialogs). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7279).

## [v0.37.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.37.0-victorialogs)

Released at 2024-10-18

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add ability to hide hits chart. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7117).
* FEATURE: add basic [alerting rules](https://github.com/zzylol/VictoriaMetrics-cluster/blob/master/deployment/docker/rules/alerts-vlogs.yml) for VictoriaLogs process. See details at [monitoring docs](https://docs.victoriametrics.com/victorialogs/#monitoring).
...
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix display of hits chart. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7133).

## [v0.36.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.36.0-victorialogs)
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add the ability to cancel running queries. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7097).

* BUGFIX: avoid possible panic when logs for a new day are ingested during execution of concurrent queries.
* BUGFIX: avoid panic at `lib/logstorage.(*blockResultColumn).forEachDictValue()` when the query contains [stats with additional filters](https://docs.victoriametrics.com/victorialogs/logsql/#stats-with-additional-filters). The panic has been introduced in [v0.33.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.33.0-victorialogs) in [this commit](https://github.com/zzylol/VictoriaMetrics-cluster/commit/a350be48b68330ee1a487e1fb09b002d3be45163).
...
* BUGFIX: [vmui](https://docs.victoriametrics.com/#vmui): fix error messages rendering from overflowing the screen with long messages. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7207).

## [v0.35.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.35.0-victorialogs)
...
## [v0.34.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.34.0-victorialogs)
...
## [v0.33.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.33.0-victorialogs)
...
* BUGFIX: [`count_uniq` stats function](https://docs.victoriametrics.com/victorialogs/logsql/#count_uniq-stats): do not count field values, which aren't matched by the used filters. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7152).

## [v0.32.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.32.1-victorialogs)
...
## [v0.32.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.32.0-victorialogs)
...
## [v0.31.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.31.0-victorialogs)

Released at 2024-09-27

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): improved readability of staircase graphs and tooltip usability. See [this comment](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6545#issuecomment-2336805237).
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): simplify query input by adding only the label name when `ctrl`+clicking the line legend. See [this comment](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6545#issuecomment-2336805237).
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): keep selected columns in table view on page reloads. Before, selected columns were reset on each update. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7016).
...
## [v0.30.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.30.1-victorialogs)
...
## [v0.30.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.30.0-victorialogs)

Released at 2024-09-27

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add button for enabling auto refresh, similarly to VictoriaMetrics vmui. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7017).
* FEATURE: drop logs without [`_msg`](https://docs.victoriametrics.com/victorialogs/keyconcepts/#message-field) field or with empty `_msg` field, since this field is required to be non-empty in [VictoriaLogs data model](https://docs.victoriametrics.com/victorialogs/keyconcepts/#data-model). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6785).
* FEATURE: improve performance of analytical queries, which do not need reading the `_time` field. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/7070).
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add support for displaying the top 5 log streams in the hits graph. The remaining log streams are grouped into an "other" label. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6545).
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add fields for setting AccountID and ProjectID. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6631).
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add search functionality to the column display settings in the table. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6668).
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add the ability to select all columns in the column display settings of the table. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6668). Thanks to @yincongcyincong for the [pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6680).
* FEATURE: Allow to define ingestion parameters via headers. Supported headers - `VL-Msg-Field`,`VL-Stream-Fields`,`VL-Ignore-Fields`,`VL-Time-Field`, `VL-Debug`. See this [PR](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6443) for details.
* FEATURE: [vlinsert](https://docs.victoriametrics.com/victorialogs/): added OpenTelemetry logs ingestion support. See this [PR](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6218) for details.

* BUGFIX: properly handle Logstash requests for Elasticsearch configuration when using `outputs.elasticsearch` in Logstash pipelines. Previously, the requests could be rejected with `400 Bad Request` response. Updates [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4750).
* BUGFIX: [vmui](https://docs.victoriametrics.com/#vmui): fix `not found index.js` error when loading vmui in VictoriaLogs. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6764). Thanks to @yincongcyincong for the [pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6770).
* BUGFIX: properly execute queries with `OR` [filters](https://docs.victoriametrics.com/victorialogs/logsql/#logical-filter) for distinct [log fields](https://docs.victoriametrics.com/victorialogs/keyconcepts/#data-model). For example, `field1:foo OR field2:bar`. Previously logs matching these filters may be skipped during querying. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6554) for details. Thanks to @yincongcyincong for the [pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6556).

## [v0.28.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.28.0-victorialogs)

Released at 2024-07-10

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): show a spinner on top of bar chart until user's request is finished. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6558).
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): use compact representation of JSON lines at `JSON` tab if only a single [log field](https://docs.victoriametrics.com/victorialogs/keyconcepts/#data-model) is queried. See [this feature request](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6559).
...
## [v0.27.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.27.1-victorialogs)

Released at 2024-07-05

* BUGFIX: properly JSON-encode strings with special chars in [HTTP querying API](https://docs.victoriametrics.com/victorialogs/querying/#http-api) responses. This fixes the `error decode response: invalid character 'x' in string escape code` error in [VictoriaLogs datasource for Grafana](https://github.com/VictoriaMetrics/victorialogs-datasource/). See [this issue](https://github.com/VictoriaMetrics/victorialogs-datasource/issues/24). The issue has been introduced in the release [v0.9.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.9.0-victorialogs).

## [v0.27.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.27.0-victorialogs)
...
## [v0.26.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.26.1-victorialogs)

Released at 2024-07-01

* BUGFIX: return the proper surrounding logs for [`stream_context` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#stream_context-pipe) when additional [pipes](https://docs.victoriametrics.com/victorialogs/logsql/#pipes) are put after the `stream_context` pipe. This has been broken in [v0.26.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.26.0-victorialogs).

## [v0.26.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.26.0-victorialogs)
...
## [v0.25.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.25.0-victorialogs)
...
* FEATURE: add ability to return top `N` `"fields"` groups from  [`/select/logsql/hits` HTTP endpoint](https://docs.victoriametrics.com/victorialogs/querying/#querying-hits-stats), by specifying `fields_limit=N` query arg. This query arg is going to be used in [this feature request](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6545).

* BUGFIX: fix `runtime error: index out of range [0] with length 0` panic when empty lines are ingested via [Syslog format](https://docs.victoriametrics.com/victorialogs/data-ingestion/syslog/) by Cisco controllers. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6548).

## [v0.24.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.24.0-victorialogs)
...
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix the update of the relative time range when `Execute Query` is clicked. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6345).

## [v0.23.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.23.0-victorialogs)
...
* BUGFIX: properly parse timestamps with timezones during [data ingestion](https://docs.victoriametrics.com/victorialogs/data-ingestion/) and [querying](https://docs.victoriametrics.com/victorialogs/querying/). This has been broken in [v0.20.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.20.0-victorialogs). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6508).

## [v0.22.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.22.0-victorialogs)
...
## [v0.21.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.21.0-victorialogs)

Released at 2024-06-20

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add a bar chart displaying the number of log entries over a time range. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6404).
...
## [v0.20.2](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.20.2-victorialogs)

Released at 2024-06-18

* BUGFIX: properly parse timestamps with nanosecond precision for logs ingested via [jsonline format](https://docs.victoriametrics.com/victorialogs/data-ingestion/#json-stream-api). The bug has been introduced in [v0.20.0 release](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.20.0-victorialogs).

## [v0.20.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.20.1-victorialogs)
...
## [v0.20.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.20.0-victorialogs)
...
## [v0.19.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.19.0-victorialogs)
...
* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): add markdown support to the `Group` view. See [this pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6292).

* BUGFIX: return back the improved performance for queries with `*` filters (aka `SELECT *`). This has been broken in [v0.16.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.16.0-victorialogs).

## [v0.18.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.18.0-victorialogs)

Released at 2024-06-06

* FEATURE: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): improve displaying of logs. See [this pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/6419) and the following issues: [6408](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6408), [6405](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6405), [6406](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6406) and [6407](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6407).
...
## [v0.17.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.17.0-victorialogs)
...
## [v0.16.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.16.0-victorialogs)
...
## [v0.15.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.15.0-victorialogs)
...
## [v0.14.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.14.0-victorialogs)
...
## [v0.13.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.13.0-victorialogs)
...
## [v0.12.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.12.1-victorialogs)
...
## [v0.11.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.11.0-victorialogs)
...
## [v0.10.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.10.0-victorialogs)
...
## [v0.9.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.9.1-victorialogs)

Released at 2024-05-22

* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix loading web UI, which has been broken in [v0.9.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.9.0-victorialogs).

## [v0.9.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.9.0-victorialogs)
...
* BUGFIX: fix `invalid memory address or nil pointer dereference` panic when using [`extract`](https://docs.victoriametrics.com/victorialogs/logsql/#extract-pipe), [`unpack_json`](https://docs.victoriametrics.com/victorialogs/logsql/#unpack_json-pipe) or [`unpack_logfmt`](https://docs.victoriametrics.com/victorialogs/logsql/#unpack_logfmt-pipe) pipes. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6306).
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): fix an issue where logs with long `_msg` values might not display. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6281).
* BUGFIX: properly handle time range boundaries with millisecond precision. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6293).

## [v0.8.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.8.0-victorialogs)
...
## [v0.7.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.7.0-victorialogs)
...
## [v0.6.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.6.1-victorialogs)
...
* BUGFIX: properly return matching logs in [streams](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields) with small number of entries. Previously they could be skipped. The issue has been introduced in [the release v0.6.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.6.0-victorialogs).
* BUGFIX: fix `runtime error: index out of range` panic when using [`sort` pipe](https://docs.victoriametrics.com/victorialogs/logsql/#sort-pipe) like `_time:1h | sort by (_time)`. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6258).

## [v0.6.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.6.0-victorialogs)
...
* BUGFIX: prevent from returning log entries with empty `_stream` field in the form `"_stream":""` in [search query results](https://docs.victoriametrics.com/victorialogs/querying/). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/6042).

## [v0.5.2](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.5.2-victorialogs)

Released at 2024-04-11

* BUGFIX: properly register new [log streams](https://docs.victoriametrics.com/victorialogs/keyconcepts/#stream-fields) under high data ingestion rate. The issue has been introduced in [v0.5.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.5.0-victorialogs).

## [v0.5.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.5.1-victorialogs)

Released at 2024-04-04

* BUGFIX: properly apply time range filter for queries containing [`OR` operators](https://docs.victoriametrics.com/victorialogs/logsql/#logical-filter). See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/5920).
* BUGFIX: do not log debug lines `DEBUG: start trimLines` and `DEBUG: end trimLines`. This bug has been introduced in [v0.5.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.5.0-victorialogs) in [this commit](https://github.com/zzylol/VictoriaMetrics-cluster/commit/0514091948cf8e00e42f44318c0e5e5b63b6388f).

## [v0.5.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.5.0-victorialogs)

Released at 2024-03-01

* FEATURE: support the ability to limit the number of returned log entries from [HTTP querying API](https://docs.victoriametrics.com/victorialogs/querying/#http-api) by passing `limit` query arg. Previously all the matching log entries were returned until closing the response stream. See [this feature request](https://github.com/zzylol/VictoriaMetrics-cluster/issues/5674). Thanks to @dmitryk-dk for [the pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/5778).

* BUGFIX: do not panic on incorrect regular expression in [stream filter](https://docs.victoriametrics.com/victorialogs/logsql/#stream-filter). Thanks to @XLONG96 for [the bugfix](https://github.com/zzylol/VictoriaMetrics-cluster/pull/5897).
* BUGFIX: properly determine when the assisted merge is needed. Previously the logs for determining whether the assisted merge is needed was broken. This could lead to too big number of parts under high data ingestion rate. Thanks to @lujiajing1126 for [the fix](https://github.com/zzylol/VictoriaMetrics-cluster/pull/5447).
* BUGFIX: properly stop execution of aborted query when the query doesn't contain [`_stream` filter](https://docs.victoriametrics.com/victorialogs/logsql/#stream-filter). Previously such a query could continue consuming resources after being aborted by the client. Thanks to @z-anshun for [the fix](https://github.com/zzylol/VictoriaMetrics-cluster/pull/5400).

## [v0.4.2](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.4.2-victorialogs)

Released at 2023-11-15

* BUGFIX: properly locate logs for the [requested streams](https://docs.victoriametrics.com/victorialogs/logsql/#stream-filter). Previously logs for some streams may be missing in query results. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4856). Thanks to @XLONG96 for [the fix](https://github.com/zzylol/VictoriaMetrics-cluster/pull/5295)!
* BUGFIX: [web UI](https://docs.victoriametrics.com/victorialogs/querying/#web-ui): properly sort found logs by time. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/5300).

## [v0.4.1](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.4.1-victorialogs)

Released at 2023-10-04

* BUGFIX: fix the free space verification process in VictoriaLogs that was erroneously shifting to read-only mode, despite there being sufficient free space available. See [this](https://github.com/zzylol/VictoriaMetrics-cluster/issues/5112) issue.

## [v0.4.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.4.0-victorialogs)

Released at 2023-10-03

* FEATURE: add `-elasticsearch.version` command-line flag, which can be used for specifying Elasticsearch version returned by VictoriaLogs to Filebeat at [elasticsearch bulk API](https://docs.victoriametrics.com/victorialogs/data-ingestion/#elasticsearch-bulk-api). This helps resolving [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4777).
...
* FEATURE: add `-insert.maxFieldsPerLine` command-line flag, which can be used for limiting the number of fields per line in logs sent to VictoriaLogs via ingestion protocols. This helps to avoid issues like [this](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4762).
* FEATURE: expose `vl_http_request_duration_seconds` histogram at the [/metrics](https://docs.victoriametrics.com/victorialogs/#monitoring) page. Thanks to @crossoverJie for [this pull request](https://github.com/zzylol/VictoriaMetrics-cluster/pull/4934).
* FEATURE: add support of `-storage.minFreeDiskSpaceBytes` command-line flag to allow switching to read-only mode when running out of disk space at `-storageDataPath`. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4737).

* BUGFIX: fix possible panic when no data is written to VictoriaLogs for a long time. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4895). Thanks to @crossoverJie for filing and fixing the issue.
* BUGFIX: add `/insert/loki/ready` endpoint, which is used by Promtail for healthchecks. This should remove `unsupported path requested: /insert/loki/ready` warning logs. See [this comment](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4762#issuecomment-1690966722).
* BUGFIX: prevent from panic during background merge when the number of columns in the resulting block exceeds the maximum allowed number of columns per block. See [this issue](https://github.com/zzylol/VictoriaMetrics-cluster/issues/4762).

## [v0.3.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.3.0-victorialogs)
...
## [v0.2.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.2.0-victorialogs)
...
## [v0.1.0](https://github.com/zzylol/VictoriaMetrics-cluster/releases/tag/v0.1.0-victorialogs)
