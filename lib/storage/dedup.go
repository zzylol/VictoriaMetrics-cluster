...
// https://github.com/zzylol/VictoriaMetrics-cluster/issues/5587
func DeduplicateSamples(srcTimestamps []int64, srcValues []float64, dedupInterval int64) ([]int64, []float64) {
	...
	for i, ts := range srcTimestamps[1:] {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/3333
		j := i
		...
		for j > 0 && srcTimestamps[j-1] == tsPrev {
			...
			if decimal.IsStaleNaN(srcValues[j]) {
				...
				// see https://github.com/zzylol/VictoriaMetrics-cluster/issues/7674
				vPrev = srcValues[j]
				break
			}
