...
type Group struct {
	...
	// see https://github.com/zzylol/VictoriaMetrics-cluster/issues/5155
...
func delayBeforeStart(ts time.Time, key uint64, interval time.Duration, offset *time.Duration) time.Duration {
	...
	// see https://github.com/zzylol/VictoriaMetrics-cluster/issues/3409.
	if offset != nil {
...
func (g *Group) adjustReqTimestamp(timestamp time.Time) time.Time {
	...
	if g.evalAlignment == nil || *g.evalAlignment {
		...
		// and https://github.com/zzylol/VictoriaMetrics-cluster/issues/1232
