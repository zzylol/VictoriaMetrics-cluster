...
var (
	...
	maxSamplesPerQuery   = flag.Int("search.maxSamplesPerQuery", 1e12, "The maximum number of raw samples a single query can process across all time series. This protects from heavy queries, which select unexpectedly high number of raw samples. See also -search.maxSamplesPerSeries")
...
func timeseriesWorker(qt *querytracer.Tracer, workChs []chan *timeseriesWork, workerID uint) {
	...
	for i := uint(1); i < uint(len(workChs)); i++ {
		...
		for len(ch) > 0 {
			...
			// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/3966#issuecomment-1483208419
...
func GraphiteTags(qt *querytracer.Tracer, accountID, projectID uint32, denyPartialResponse bool, filter string, limit int, deadline searchutils.Deadline) ([]string, bool, error) {
	...
	for i := range labels {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/942
...
const maxEvalResultSize = 1024 * 1024 * 1024
...
func Init(storage_addrs []string) {
