...
import (
	...
	"github.com/zzylol/VictoriaMetrics-cluster/app/vmselect/querystats"
	"github.com/zzylol/VictoriaMetrics-cluster/app/vmselect/searchutils"
	"github.com/zzylol/VictoriaMetrics-cluster/lib/auth"
	"github.com/zzylol/VictoriaMetrics-cluster/lib/bufferedwriter"
	...
	"github.com/zzylol/VictoriaMetrics-cluster/lib/httputils"
...
func SeriesHandler(qt *querytracer.Tracer, startTime time.Time, at *auth.Token, w http.ResponseWriter, r *http.Request) error {
	...
	// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/91
...
func QueryHandler(qt *querytracer.Tracer, startTime time.Time, at *auth.Token, w http.ResponseWriter, r *http.Request) error {
	...
	if !httputils.GetBool(r, "nocache") && ct-start < queryOffset && start-ct < queryOffset {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/241
		startPrev := start
...
func queryRangeHandler(qt *querytracer.Tracer, startTime time.Time, at *auth.Token, w http.ResponseWriter, query string,
	...
	// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/153
...
func adjustLastPoints(tss []netstorage.Result, start, end int64) []netstorage.Result {
	for i := range tss {
		...
		if j >= 0 && timestamps[j] > end {
			...
			// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/625
			continue
		}
...
func getLatencyOffsetMilliseconds(r *http.Request) (int64, error) {
	...
	if d < 0 {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/2061#issuecomment-1299109836
		d = 0
	}
...
func getCommonParamsInternal(r *http.Request, startTime time.Time, requireNonEmptyMatch, isLabelsAPI bool) (*commonParams, error) {
	...
	// See https://github.com/zzylol/VictoriaMetrics-cluster/blob/ea06d2fd3ccbbb6aa4480ab3b04f7b671408be2a/lib/storage/table.go#L378
	// This should fix possible timestamp overflow - see https://github.com/zzylol/VictoriaMetrics-cluster/issues/2669
