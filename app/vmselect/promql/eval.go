...
func AdjustStartEnd(start, end, step int64) (int64, int64) {
	if *disableCache {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/563
		return start, end
	}
	points := (end-start)/step + 1
...
func execBinaryOpArgs(qt *querytracer.Tracer, ec *EvalConfig, exprFirst, exprSecond metricsql.Expr, be *metricsql.BinaryOpExpr) ([]*timeseries, []*timeseries, error) {
	if !canPushdownCommonFilters(be) {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/2886
	...
	if len(tssFirst) == 0 && !strings.EqualFold(be.Op, "or") {
		...
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/3349
		return nil, nil, nil
	}
...
func evalRollupFunc(qt *querytracer.Tracer, ec *EvalConfig, funcName string, rf rollupFunc, args []interface{}, expr metricsql.Expr,
	re *metricsql.RollupExpr, iafc *incrementalAggrFuncContext) ([]*timeseries, error) {
	if re.At == nil {
		return evalRollupFuncWithoutAt(qt, ec, funcName, rf, args, expr, re, iafc)
	}
	...
	tss, err := evalRollupFuncWithoutAt(qt, ecNew, funcName, rf, args, expr, re, iafc)
	if err != nil {
		return nil, err
	}
...
func evalRollupFuncWithoutAt(qt *querytracer.Tracer, ec *EvalConfig, funcName string, rf rollupFunc, args []interface{},
	...
	if re.Offset != nil {
		...
		// See also https://github.com/zzylol/VictoriaMetrics-cluster/issues/976
	}
	if funcName == "rollup_candlestick" {
		// Automatically apply `offset -step` to `rollup_candlestick` function
		// in order to obtain expected OHLC results.
		// See https://github.com/zzylol/VictoriaMetrics-cluster/issues/309#issuecomment-582113462
		step := ecNew.Step
		ecNew = copyEvalConfig(ecNew)
		ecNew.Start += step
		ecNew.End += step
	...
	if me, ok := re.Expr.(*metricsql.MetricExpr); ok {
		rvs, err = evalRollupFuncWithMetricExpr(qt, ecNew, funcName, rf, args, expr, me, iafc, re.Window)
	} else {
		if iafc != nil {
...
// This follows Prometheus logic - see https://github.com/zzylol/VictoriaMetrics-cluster/issues/2130
...
func evalInstantRollup(qt *querytracer.Tracer, ec *EvalConfig, funcName string, rf rollupFunc, args []interface{},
	...
	evalAt := func(qt *querytracer.Tracer, timestamp, window int64) ([]*timeseries, error) {
		...
		tss, err := evalRollupFuncNoCache(qt, ecCopy, funcName, rf, args, expr, me, iafc, window, pointsPerSeries)
		if err != nil {
			return nil, err
		}
...
func evalRollupFuncNoCache(qt *querytracer.Tracer, ec *EvalConfig, funcName string, rf rollupFunc, args []interface{},
	...
	if err_sketch == nil {
		...
		if err == nil && funcNameID >= 1 && funcNameID <= 13 && funcName != "count_over_time" {
			...
			logger.Infof("!!! Returned Eval timeseries: funcName=%s, tss_len=%d, isCovered=%d, err:%s", funcName, len(ts_results), isCovered, err)
