...
// sketchNode is a client sending data to vmsketch node.
type sketchNode struct {
	// isBroken is set to true if the given vmsketch node is temporarily unhealthy.
	// In this case the data is re-routed to the remaining healthy vmsketch nodes.
	isBroken atomic.Bool

	// isReadOnly is set to true if the given vmsketch node is read only
	// In this case the data is re-routed to the remaining healthy vmsketch nodes.
	...
	// Buffer with data that needs to be written to the sketch node.
	...
	// bc is a single connection to vmsketch for data transfer.
	...
	// The number of dial errors to vmsketch node.
	dialErrors *metrics.Counter

	// The number of handshake errors to vmsketch node.
	handshakeErrors *metrics.Counter

	// The number of connection errors to vmsketch node.
	connectionErrors *metrics.Counter

	// The number of rows pushed to sketchNode with push method.
	rowsPushed *metrics.Counter

	// The number of rows sent to vmsketch node.
	...
	// The number of rows rerouted from the given vmsketch node
	// to healthy nodes when the given node was unhealthy.
	rowsReroutedFromHere *metrics.Counter

	// The number of rows rerouted to the given vmsketch node
	// from other nodes when they were unhealthy.
	rowsReroutedToHere *metrics.Counter

	// The total duration spent for sending data to vmsketch node.
	// This metric is useful for determining the saturation of vminsert->vmsketch link.
...
type sketchNodesBucket struct {
	...
	// nodesHash is used for consistently selecting a sketch node by key.
	nodesHash *consistentHash

	// sns is a list of sketch nodes.
	sns []*sketchNode
...
// sketchNodes contains a list of vmsketch node clients.
...
func setSketchNodesBucket(sknb *sketchNodesBucket) {
	sketchNodes.Store(sknb)
}
...
func initSketchNodes(addrs []string, hashSeed uint64) *sketchNodesBucket {
	...
	maxBufSizePerSketchNode = memory.Allowed() / 8 / len(sns)
	if maxBufSizePerSketchNode > consts.MaxInsertPacketSizeForVMInsert {
		maxBufSizePerSketchNode = consts.MaxInsertPacketSizeForVMInsert
	}
...
func mustStopSketchNodes(sknb *sketchNodesBucket) {
	close(sknb.stopCh)
	for _, sn := range sknb.sns {
		sn.brCond.Broadcast()
	}
	sknb.wg.Wait()
	metrics.UnregisterSet(sknb.ms, true)
}

func sendSketchBufToReplicasNonblocking(snb *sketchNodesBucket, br *bufRows, snIdx, replicas int) bool {
	usedSketchNodes := make(map[*sketchNode]struct{}, replicas)
	sns := snb.sns
...
var sketchNodesBecameAvailableLogger = logger.WithThrottler("sketchNodesBecameAvailable", 5*time.Second)

var noSketchNodesLogger = logger.WithThrottler("sketchNodesUnavailable", 5*time.Second)

func getNotReadySketchNodeIdxs(snb *sketchNodesBucket, dst []int, snExtra *sketchNode) []int {
	dst = dst[:0]
...
func (sn *sketchNode) trySendBuf(buf []byte, rows int) bool {
	if !sn.isReady() {
	...
	sent := false
	sn.brLock.Lock()
	if sn.isReady() && len(sn.br.buf)+len(buf) <= maxBufSizePerSketchNode {
...
var errSketchReadOnly = errors.New("storage node is read only")

func (sn *sketchNode) isReady() bool {
...
func (sn *sketchNode) push(snb *sketchNodesBucket, buf []byte, rows int) error {
	if len(buf) > maxBufSizePerSketchNode {
		logger.Panicf("BUG: len(buf)=%d cannot exceed %d", len(buf), maxBufSizePerSketchNode)
	}
	...
	if err := sn.rerouteBufToOtherSketchNodes(snb, buf, rows); err != nil {
		return fmt.Errorf("error when re-routing rows from %s: %w", sn.dialer.Addr(), err)
	}
	return nil
}

func getNotReadySketchNodeIdxsBlocking(snb *sketchNodesBucket, dst []int) []int {
	dst = getNotReadySketchNodeIdxs(snb, dst[:0], nil)
	sns := snb.sns
	if len(dst) < len(sns) {
		return dst
	}
	noSketchNodesLogger.Warnf("all the vmstorage nodes are unavailable; stopping data processing util at least a single node becomes available")
	for {
		...
		if availableNodes := len(sns) - len(dst); availableNodes > 0 {
			sketchNodesBecameAvailableLogger.Warnf("%d vmstorage nodes became available, so continue data processing", availableNodes)
			return dst
		}
	}
}

// rerouteRowsToReadySketchNodes reroutes src from not ready snSource to ready storage nodes.
//
// The function blocks until src is fully re-routed.
func rerouteRowsToReadySketchNodes(snb *sketchNodesBucket, snSource *sketchNode, src []byte) (int, error) {
	reroutesTotal.Inc()
	rowsProcessed := 0
	var idxsExclude, idxsExcludeNew []int
	nodesHash := snb.nodesHash
	sns := snb.sns
	idxsExclude = getNotReadySketchNodeIdxsBlocking(snb, idxsExclude[:0])
	...
	again:
		...
		idxsExcludeNew = getNotReadySketchNodeIdxs(snb, idxsExcludeNew[:0], sn)
...
// reouteRowsToFreeSketchNodes re-routes src from snSource to other storage nodes.
//
...
func rerouteRowsToFreeSketchNodes(snb *sketchNodesBucket, snSource *sketchNode, src []byte) (int, error) {
	if *disableRerouting {
		logger.Panicf("BUG: disableRerouting must be disabled when calling rerouteRowsToFreeSketchNodes")
	}
	sns := snb.sns
	if len(sns) < 2 {
		logger.Panicf("BUG: the number of storage nodes is too small for calling rerouteRowsToFreeSketchNodes: %d", len(sns))
	}
	reroutesTotal.Inc()
	rowsProcessed := 0
	var idxsExclude []int
	nodesHash := snb.nodesHash
	idxsExclude = getNotReadySketchNodeIdxs(snb, idxsExclude[:0], snSource)
	var mr storage.MetricRow
	for len(src) > 0 {
		tail, err := mr.UnmarshalX(src)
		if err != nil {
...
var cannotCloseSketchNodeConnLogger = logger.WithThrottler("cannotCloseSketchNodeConn", 5*time.Second)

var cannotSendBufsLoggerSketch = logger.WithThrottler("cannotSendBufRowsSketch", 5*time.Second)

func sendToConnSketch(bc *handshake.BufferedConn, buf []byte) error {
	// if len(buf) == 0, it must be sent to the vmstorage too in order to check for vmstorage health
	// See checkReadOnlyMode() and https://github.com/zzylol/VictoriaMetrics-cluster/issues/4870
	...
	sizeBuf := sizeBufPoolSketch.Get()
	defer sizeBufPoolSketch.Put(sizeBuf)
...
var sizeBufPoolSketch bytesutil.ByteBufferPool

func (sn *sketchNode) sendBufRowsNonblocking(br *bufRows) bool {
	if !sn.isReady() {
		return false
	}
	...
	startTime := time.Now()
	err := sendToConnSketch(sn.bc, br.buf)
	...
	cannotSendBufsLoggerSketch.Warnf("cannot send %d bytes with %d rows to -sketchNode=%q: %s; closing the connection to sketchNode and "+
		"re-routing this data to healthy storage nodes", len(br.buf), br.rows, sn.dialer.Addr(), err)
	if err = sn.bc.Close(); err != nil {
		cannotCloseSketchNodeConnLogger.Warnf("cannot close connection to sketchNode %q: %s", sn.dialer.Addr(), err)
	}
	sn.bc = nil
...
var dropSketchSamplesOnOverloadLogger = logger.WithThrottler("droppedSketchSamplesOnOverload", 5*time.Second)

func (sn *sketchNode) rerouteBufToOtherSketchNodes(snb *sketchNodesBucket, buf []byte, rows int) error {
	sns := snb.sns
	sn.brLock.Lock()
again:
	select {
	...
	if !sn.isReady() {
		...
		rowsProcessed, err := rerouteRowsToReadySketchNodes(snb, sn, buf)
		rows -= rowsProcessed
		if err != nil {
	...
	if len(sn.br.buf)+len(buf) <= maxBufSizePerSketchNode {
	...
	sn.brLock.Unlock()
	rowsProcessed, err := rerouteRowsToFreeSketchNodes(snb, sn, buf)
	rows -= rowsProcessed
	if err != nil {
...
func (sn *sketchNode) run(snb *sketchNodesBucket, snIdx int) {
	...
	for !mustStop {
		...
		for !sendSketchBufToReplicasNonblocking(snb, &br, snIdx, replicas) {
			d := timeutil.AddJitterToDuration(time.Millisecond * 200)
			t := timerpool.Get(d)
			select {
			case <-sn.stopCh:
...
func (sn *sketchNode) sendBufMayBlock(buf []byte) bool {
	sn.brLock.Lock()
	for len(sn.br.buf)+len(buf) > maxBufSizePerSketchNode {
		select {
		case <-sn.stopCh:
			sn.brLock.Unlock()
			return false
...
func (sn *sketchNode) readOnlyChecker() {
...
func (sn *sketchNode) checkReadOnlyMode() {
	if !sn.isReadOnly.Load() {
	...
	err := sendToConnSketch(sn.bc, nil)
	if err == nil {
	...
	logger.Errorf("cannot check storage readonly mode for -sketchNode=%q: %s", sn.dialer.Addr(), err)
...
func (sn *sketchNode) checkHealth() {
	sn.bcLock.Lock()
	defer sn.bcLock.Unlock()
	...
	if err != nil {
		...
		if sn.lastDialErr == nil {
			...
			sn.lastDialErr = err
			logger.Warnf("cannot dial sketchNode %q: %s", sn.dialer.Addr(), err)
		}
		return
	}
	logger.Infof("successfully dialed -sketchNode=%q", sn.dialer.Addr())
	sn.lastDialErr = nil
	sn.bc = bc
...
func (sn *sketchNode) dial() (*handshake.BufferedConn, error) {
